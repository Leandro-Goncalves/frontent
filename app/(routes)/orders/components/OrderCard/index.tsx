...
const alertDataObject: alertDataObjectFunction = (
  ...
  return {
    ...
    [OrderStatus.pending]: {
      text: "Pagamento pendente, clique no card para pagar (o pedido será cancelado em 30 minutos)",
      color: "#FFCC6D",
    },
...
export const OrderCard: React.FC<OrderCardProps> = ({ order }) => {
  ...
  const handleClick = () => {
    ...
    if (order.status === OrderStatus.expired) canceledAction();
    if (order.status === OrderStatus.success) successAction(order.tracking);
  };

  return (
    <button
      className="group rounded-lg overflow-hidden border-primary/20 cursor-pointer border-[2px] hover:border-primary transition-all"
      onClick={handleClick}
    >
      {alertData && (
      ...
      {order.products.map((product, index) => {
        ...
        return (
          <div
            key={product.id}
            className="p-4 text-start border-t-[2px] border-primary border-opacity-10 group-hover:border-opacity-100 transition-all"
            style={{
